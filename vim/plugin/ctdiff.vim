"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
" ctdiff (C) 2010 Ozkan Dikmen (based on svndiff by 'Ico Doornekamp')
"
" Usage
" -----
"   ""    : Equivalent to running 'ctdiff file1'
"   "pre" : Equivalent to running 'ctdiff -pre file1'
"
"
"""""""""""""""" Notes below apply to svndiff version of this file
"
...
function s:Svndiff_update(...)
    ...
    if ! has_key(s:rcs_type, fname)
        echom "ctdiff: Warning, file " . fname . " is not managed by subversion or git"
...
function CtdiffFull(...)
    ...
    if cmd == ''
        let diff = system("ctdiff " . fname . "&")
    end

    if cmd == 'pre'
        let diff = system("ctdiff -pre " . fname . "&")
    end
